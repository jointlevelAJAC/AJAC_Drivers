...
#include "h7_task.h"
H7_Board_TypeDef* usb_local_h7_board;

void set_h7board_usb(void) {
  usb_local_h7_board = get_h7_board();
}
...
__ALIGN_BEGIN static uint8_t CUSTOM_HID_ReportDesc_HS[USBD_CUSTOM_HID_REPORT_DESC_SIZE] __ALIGN_END =
{
  /* USER CODE BEGIN 1 */
    ...
    0x96, 0x78, 0x01,               //   REPORT_COUNT (376)
    0x75, 0x08,                    //   REPORT_SIZE (8)
    0x91, 0x00,                    //   OUTPUT (Data,Ary,Abs)
    0x09, 0x00,                    //   USAGE (Undefined)
    0x15, 0x00,                    //   LOGICAL_MINIMUM (0)
    0x26, 0xff, 0x00,              //   LOGICAL_MAXIMUM (255)
    0x95, 0x78, 0x01,              //   REPORT_COUNT (156)
    0x75, 0x08,                    //   REPORT_SIZE (8)
...
static int8_t CUSTOM_HID_OutEvent_HS(uint8_t event_idx, uint8_t state)
{
  ...
  usb_local_h7_board->receive_usb_cmd((uint8_t*)(hhid->Report_buf));
  usb_local_h7_board->cmd_update = 1;
  usb_local_h7_board->hit_miss = 0;
  //LL_GPIO_TogglePin(PIN_TEST1_GPIO_Port, PIN_TEST1_Pin);
  return (USBD_OK);
  /* USER CODE END 10 */
}
